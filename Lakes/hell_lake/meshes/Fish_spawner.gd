...
@export var surface_offset: float = 0.0   # offset above sampled lake surface
@export var max_depth_offset: float = -10.0  # offset below sampled lake surface

var multi_mesh_instance: MultiMeshInstance3D
var fish_data: Array = []
var spatial_grid: Dictionary = {}

# Lake mesh sampling state
var _mdt: MeshDataTool
var _valid_tris: Array = []
var _valid_areas: Array = []
var _total_area: float = 0.0
var _lake_surface_y: float = 0.0  # average local Y of the sampled lake mesh
...
func _ready():
	...
	_sample_lake_mesh()
	if _valid_tris.is_empty():
		push_error("Fish spawner: no lake triangles sampled, aborting")
		return
...
func _sample_lake_mesh():
	if not mesh:
		push_error("Fish spawner: this node has no mesh assigned")
		return
	_mdt = MeshDataTool.new()
	if _mdt.create_from_surface(mesh, 0) != OK:
		push_error("Fish spawner: MeshDataTool failed on lake mesh")
		return
	var face_count = _mdt.get_face_count()
	var y_sum = 0.0
	var y_count = 0
	for f in face_count:
		...
		for v in [a, b, c]:
			...
			y_count += 1
	if y_count > 0:
		_lake_surface_y = y_sum / y_count
	print("Fish spawner: sampled ", _valid_tris.size(), " lake triangles, surface_y=", _lake_surface_y)

func _random_point_on_lake() -> Vector3:
	var tri = _pick_weighted_tri(_valid_tris, _valid_areas, _total_area)
	...
	local_pos.y = _lake_surface_y + randf_range(surface_offset - 1.0, max_depth_offset + 1.0)
	return local_pos
...
func update_fish(delta):
	for i in range(fish_data.size()):
		...
		var surface_y = _lake_surface_y + surface_offset
		var max_depth = _lake_surface_y + max_depth_offset
