...
var selected_config: VehicleConfig = null
...
func _load_all_configs() -> void:
	...
	while file_name != "":
		if file_name.ends_with(".tres"):
			print("Found tres: " + file_name)
			var config = load(CONFIGS_PATH + file_name)
			print("Loaded as: " + str(config))
			if config is VehicleConfig:
				all_configs.append(config)
				print("Added: " + config.display_name)
		file_name = dir.get_next()
	dir.list_dir_end()
	print("Total configs loaded: " + str(all_configs.size()))
...
func _populate_tree(configs: Array) -> void:
	...
	for class_name_str in classes:
		...
		print("Tree item count: " + str(tree.get_root().get_child_count()))
		print("Classes found: " + str(classes.keys()))
...
func _on_tree_item_selected() -> void:
	...
	car_label.text = config.display_name
	race_selected_btn.disabled = false
	_load_preview(config.scene_path)
...
func _load_last_car() -> void:
	if not GameManager.last_raced_config:
		...
		return
	car_label.text = GameManager.last_raced_config.display_name
	_load_preview(GameManager.last_raced_config.scene_path)

func _on_race_selected_pressed() -> void:
	if not selected_config:
		return
	GameManager.player_car_config = selected_config
	GameManager.last_raced_config = selected_config
