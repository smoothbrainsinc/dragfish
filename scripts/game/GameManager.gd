...
# Lane assignment (1 = farthest right, counting up going left)
...
# Spawn markers, index 0 = lane 1
var lane_spawns: Array[Marker3D] = []
...
## Set spawn point markers. Array must be pre-sorted lane 1..N (index 0 = lane 1).
func set_spawn_markers(markers: Array[Marker3D]) -> void:
	if markers.is_empty():
		push_error("[GameManager] Empty spawn marker array provided!")
		return
	lane_spawns = markers
	print("[GameManager] Spawn markers set: %d lanes" % markers.size())

## Spawn vehicle body only (NO script attachment here)
## RaceManager handles script attachment after add_child
func spawn_vehicle(config: VehicleConfig, is_player: bool, lane: int) -> VehicleBody3D:
	if not config or not config.is_valid():
		push_error("[GameManager] Invalid config for spawn!")
		return null
	if lane_spawns.is_empty():
		push_error("[GameManager] Spawn markers not set! Call set_spawn_markers() first.")
		return null
	if lane < 1 or lane > lane_spawns.size():
		push_error("[GameManager] Lane %d out of range (have %d lanes)!" % [lane, lane_spawns.size()])
		return null
	...
	var spawn_marker = lane_spawns[lane - 1]
	vehicle_body.set_meta("spawn_lane", lane)
	...
	print("[GameManager] Created vehicle body for lane %d" % lane)
	return vehicle_body
